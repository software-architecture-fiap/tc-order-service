...
def create_category(
    ...
    logger.info(f"Usuário {user['id']} está criando uma categoria: {category.name}")
...
def get_categories(
    ...
    if category_id:
        ...
        return [category]  # 🔹 Agora retorna uma lista, para evitar conflito no response_model
...
def update_category(
    ...
    """
    if not category_data.dict(exclude_unset=True):  # 🔹 Verifica se há dados para atualizar
        logger.warning(f"Usuário {user['id']} tentou atualizar categoria {category_id} sem fornecer dados")
        raise HTTPException(status_code=400, detail="Nenhum dado para atualização")

    logger.info(f"Usuário {user['id']} está editando a categoria {category_id}")
    
    updated_category = repository.update_category(db, category_id, category_data)
    if not updated_category:
        logger.warning(f"Tentativa de editar categoria ID {category_id} que não existe")
        raise HTTPException(status_code=404, detail="Categoria não encontrada")
